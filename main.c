...
const char * set_lst(char input_lst[]){
  ...
  for (int i = 0; i < length; i++ ){
    ...
    if (match_counter == 0){
      ...
      // store the first occurance of input_lst[i] in set_temp and increment the index so when it loops the next unique char will go in the next avaliable slot
  ...
  // printf("\n%s", set_temp); 
   // as set_temp is a local variable i've made a pointer which points towards the memory ascociated with set_temp
